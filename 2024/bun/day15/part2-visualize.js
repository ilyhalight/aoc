...
const printMap = () => {
  ...
  testMap[posY][posX] = styleText("yellow", "@");
  console.log(
    testMap
      .map((v) => v.join(""))
      .join("\n")
      .replaceAll("#", styleText("cyan", "#"))
  );
  moveId += 1;
};
...
const t = setInterval(() => {
  ...
  process.stdout.write("\x03 ");
  console.clear();
  console.log(
    `Move: ${styleText("yellow", move)} (${moveId + 1}/${moveCount})`
  );
  ...
  const [offsetX, offsetY] = moveOffset[move];
  posX += offsetX;
  posY += offsetY;
  ...
  if (!isY) {
    ...
    for (const moved of positions.values()) {
